...
from dataset_vessel import dataset_vessel
...
def train(**kwargs):
    ...
    if not os.path.exists('C:/Users/ZMS/Desktop/checkpoints/{}/{}--{}--{}/'.format(opt.model, opt.mark, opt.lr, opt.batch_size)):
        os.makedirs('C:/Users/ZMS/Desktop/checkpoints/{}/{}--{}--{}/'.format(opt.model, opt.mark, opt.lr, opt.batch_size))

    # configure model
    model = getattr(models, opt.model)()
    if opt.load_vessel_model_path:
        model.load(opt.load_vessel_model_path)
    model.to(opt.device)
    ...
    # data
    train_data = dataset_vessel(opt.train_data_root, train=True)
    val_data = dataset_vessel(opt.train_data_root, val=True)
    ...
    for epoch in range(opt.max_epoch):
        ...
        np.save('C:/Users/ZMS/Desktop/checkpoints/{}/{}--{}--{}/train_loss.npy'.format(opt.model, opt.mark, opt.lr,
                                                                                        opt.batch_size), train_loss)
        np.save('C:/Users/ZMS/Desktop/checkpoints/{}/{}--{}--{}/train_dice.npy'.format(opt.model, opt.mark, opt.lr,
                                                                                        opt.batch_size), train_DICE)
        np.save('C:/Users/ZMS/Desktop/checkpoints/{}/{}--{}--{}/val_dice.npy'.format(opt.model, opt.mark, opt.lr,
                                                                                      opt.batch_size), val_DICE)

        model.save(
            'C:/Users/ZMS/Desktop/checkpoints/{}/{}--{}--{}/{:0>4d}--{:.4f}--{:.4f}.pth'.format(
...
def test(**kwargs):
    ...
    # data
    test_data = dataset_vessel(opt.test_data_root, test=True)
    ...
    for ii, (test_input, img_path_green) in tqdm(enumerate(test_dataloader)):
        ...
        import cv2
        cv2.imwrite('/'.join(img_path_green[0].split("/")[:-1]) + "/predict_2.1.png", seg_mask)
        # cv2.imwrite(r"C:\Users\Huang Lab\Desktop\merge"+"/predict_{}.png".format(random.random()), seg_mask)
