...
from dataset_FTn import dataset_FTn
...
def train(**kwargs):
    ...
    if opt.load_FTn_model_path:
        model.load(opt.load_FTn_model_path)
    model.to(opt.device)
    ...
    # data
    train_data = dataset_FTn(opt.train_data_root, train=True)
    val_data = dataset_FTn(opt.train_data_root, val=True)
...
def test(**kwargs):
    ...
    # data
    test_data = dataset_FTn(opt.test_data_root, test=True)
    ...
    for ii, (test_input, img_path_red) in tqdm(enumerate(test_dataloader)):
        ...
        import cv2
        cv2.imwrite('/'.join(img_path_red[0].split("/")[:-1]) +"/predict_1.1.png", seg_mask)
